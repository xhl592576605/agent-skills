...
在"研发分工与里程碑"部分划分角色时，**必须**遵循以下原则：

### 已有专家技能的角色

| 角色 | 专家技能名称 | 说明 |
|------|-------------|------|
| **前端开发** | `frontend-developer-expert` | 处理 Web 前端页面、组件、路由、状态、交互实现 |
| **多平台开发** | `multi-platform-developer-expert` | 处理 iOS/Android 跨平台任务 |
| **开发协调** | `development-lead-expert` | 开发负责人，负责创建开发计划、调度子技能、跟踪进度 |

### 角色技能指派规则

**为角色指派技能时，必须按以下顺序执行：**

1. **首先查找**：使用 Glob 工具搜索 `skills/**/SKILL.md`，查看是否存在对应角色的专家技能
2. **确认匹配**：阅读技能文档的 description，确认技能职责与角色需求匹配
3. **指派技能**：如果找到匹配的专家技能，标注技能名称
4. **使用主 agent**：如果查找不到对应的专家技能，标注为"使用主 agent"

**示例：**
```
角色：后端开发
→ 查找技能：搜索 skills/*backend* / skills/*server* / skills/*api*
→ 确认结果：未找到匹配的专家技能
→ 指派方式：标注为"使用主 agent"
```
...
**重要：**
- 不得凭空捏造不存在的技能名称
- 技能列表会持续更新，每次指派前必须重新查找确认
- 后续新增专家技能后，需同步更新此对应关系表
